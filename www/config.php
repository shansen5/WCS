...
<html>
  ...
  <title>WildlifeCam Camera Configuration</title>
   ...
   <div id="main">
    ...
    <div class="container">
     <center>
      ...
      <p><a href="index.php">Videos and Images</a></p>
      <h2>Settings</h2>
      <p class="Notes">Note that these settings only effect the live camera preview, and for the current instance.  If the camera is restarted it will revert to the settings in the configuration file.  Also, they do not determine the settings for motion detection.  Motion detection uses its own configuration file.
      <p class="Notes">To make more permanent changes to the camera configuration and motion detection, use the links further down the page to upload modified configuration files.
    ...
    <div class="container">
     <div class="uploadform">
      <h2>Upload Camera Configuration</h2>
      <p class="Notes">Use this form to upload a new configuration file for the camera and its preview window.  The camera driver will need to be restarted for this new file to take effect.
      <form action="upload_config.php" method="post"
      enctype="multipart/form-data">
      <label for="file">Select configuration file:</label>
      <input type="hidden" name="MAX_FILE_SIZE" value="15000" />
      <input type="hidden" name="cfg_file" value="/etc/raspimjpeg" />
     ...
     <div class="configfilelist">
      <?php
        $files = scandir("/etc");
        echo "<h2>Backup configuration files</h2>";
        foreach($files as $file) {
          if(($file != '.') && ($file != '..')) {
            if ( fnmatch( "raspimjpeg*", $file )) {
              echo $file . "<input type='button' value='Download' onclick='window.open(\"download_config.php?cfg_dir=/etc/&file=" . $file . "\", \"_blank\");'><br>";
            }
          }
        }
      ?>
    ...
    <div class="container">
     <div class="uploadform">
      <h2>Upload Motion Detection Configuration</h2>
      <p class="Notes">Use this form to upload a new configuration file for the motion detection program.  The camera driver will need to be restarted for this new file to take effect.
      <form action="upload_config.php" method="post"
      enctype="multipart/form-data">
      <label for="file">Select configuration file:</label>
      <input type="hidden" name="MAX_FILE_SIZE" value="50000" />
      <input type="hidden" name="cfg_file" value="/etc/motion/motion.conf" />
      <input type="file" name="file" id="file">
      <input type="submit" name="submit" value="Upload">
      </form>
     </div>
     <div class="configfilelist">
      <?php
        $files = scandir("/etc/motion");
        echo "<h2>Backup configuration files</h2>";
        foreach($files as $file) {
          if(($file != '.') && ($file != '..')) {
            if ( fnmatch( "motion.conf*", $file )) {
              echo $file . "<input type='button' value='Download' onclick='window.open(\"download_config.php?cfg_dir=/etc/motion/&file=" . $file . "\", \"_blank\");'><br>";
            }
          }
        }
      ?>
